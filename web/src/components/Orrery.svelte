<script lang="ts">
  ...
  let showTrails = true;
  let materialsCache = new Map<string, THREE.ShaderMaterial>();
  ...
  // Category Colors (Matches Phase Names)
  const CAT_COLORS: Record<string, number> = {
    Sun: 0xffd700, // Gold
    Giant_Coding: 0x00ffff, // Cyan
    Giant_Logic: 0xffaa00, // Orange
    The_Nebula: 0xff00ff, // Magenta
    Asteroid_Belt: 0x888888, // Grey
    The_Void: 0xaa00ff, // Purple
    Root: 0xff0000,
    Sacral: 0xff8c00,
    Solar: 0xffd700,
    Heart: 0x00ff00,
    ...
    ThirdEye: 0x4b0082,
    Crown: 0xffffff,
    Floret_Dream: 0xff69b4,
    Floret_Witness: 0xe0ffff,
    Floret_Reflection: 0xffd700,
  };

  function getColor(cat: string): number {
    // Direct lookup or partial match
    if (CAT_COLORS[cat]) return CAT_COLORS[cat];

    // Fallback for partial matches if needed (e.g. "Giant")
    for (const key in CAT_COLORS) {
      if (cat.includes(key)) return CAT_COLORS[key];
    }
    return 0xaaaaaa;
  }
  ...
  function buildSystem(data: SystemState) {
    ...
    data.attractors.forEach((attr) => {
      ...
      div.style.color =
        typeof attr.color === "string"
          ? attr.color
          : "#" + attr.color.toString(16);
      sphere.add(label);

      attractors.push(simAttr);
    });

    // Shared Materials Cache (Uses component scope)
    materialsCache.clear();

    const createTwinkleMaterial = (
      colorHex: number | string,
      category: string,
    ) => {
      // Determine speed based on category (Chakra frequency mapping)
      let speed = 2.0;
      const cat = category.toLowerCase();
      if (cat.includes("root")) speed = 1.0; // Slow Earth
      if (cat.includes("sacral")) speed = 1.5; // Water flow
      if (cat.includes("solar")) speed = 2.0; // Fire flicker
      if (cat.includes("heart")) speed = 3.0; // Pulse (432Hz-ish relative)
      if (cat.includes("throat")) speed = 4.0; // Vibration
      if (cat.includes("third")) speed = 5.0; // Fast signal
      if (cat.includes("crown")) speed = 8.0; // High freq shimmer

      const key = `${colorHex}-${speed}`;
      ...
      const mat = new THREE.ShaderMaterial({
        ...
        fragmentShader: `
          ...
          void main() {
            // Simple rim lighting + twinkle
            float pulse = 0.7 + 0.3 * sin(uTime * uSpeed);
            
            // Rim effect (optional, makes spheres look cooler)
            float intensity = pow(0.6 - dot(vNormal, vec3(0, 0, 1.0)), 2.0);
            
            vec3 finalColor = uColor * pulse + vec3(intensity * 0.5);
    ...
    // 2. Nodes
    const nodeGeo = new THREE.SphereGeometry(0.3, 16, 16); // Slightly better res for shader
    data.nodes.forEach((n) => {
      const color = getColor(n.category);
      const mat = createTwinkleMaterial(color, n.category);
  ...
  function animate() {
    ...
    nodes.forEach((n) => {
      if (n.trailLine) n.trailLine.visible = showTrails;
    });
    attractors.forEach((a) => {
      if (a.trailLine) a.trailLine.visible = showTrails;
    });
